 ...
 */
public class MeetupDetailActivity extends ActionBarActivity implements EventResponseListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_meetup);
        markers = new HashMap<>();
    ...
    public void updateUI(EventMessage eventMessage)    {
        ...
        try {
            ...
            JSONObject locationData = elements.getJSONObject(0);
            distance = locationData.getJSONObject("distance").get("text").toString();
            time = locationData.getJSONObject("duration").get("text").toString();
        } catch (JSONException e) {
            e.printStackTrace();
        }

        assert user != null;
        statusUpdate = user.getName() + " will reach in " + time + " from " + distance;
        markers.get(user).setPosition(new LatLng(latitude, longitude));
        markers.get(user).setTitle(statusUpdate);
    }
