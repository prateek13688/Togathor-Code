...
public class ServersListActivity extends ListActivity {
	...
	private void getServers(){
		CouchDB.findAllServersAsync(new ResultListener<List<Server>>() {
			...
			@Override
			public void onResultsSucceeded(List<Server> result) {
				mServersList = result;
				fillData();
			}
	...
	private void setServerUrlAndName(String name, String url){
		Togathor.getInstance().setBaseUrl(url, name);
		
		CouchDB.setAuthUrl(Togathor.getInstance().getBaseUrlWithSufix(Const.AUTH_URL));
		CouchDB.setUrl(Togathor.getInstance().getBaseUrlWithApi());
	}
	...
	class ServerListAdapter extends ArrayAdapter<Server>{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(view == null){
				...
				holder.serverName.setTypeface(Togathor.getTfMyriadPro());
				holder.serverUrl.setTypeface(Togathor.getTfMyriadPro());
